...
router.post('/data/update', async (req, res) => {
  try {
    ...
    const updatedStockData = req.body.map(stock => {
      ...
      addOrder(ticker, 'buy', 1, buyP, `LP-${ticker}`);
      addOrder(ticker, 'sell', 1, sellP, `LP-${ticker}`);
...
const getNewPrices = async (stock, newX, newY) => {
  ...
  if (x > 0) {
    ...
    await addOrder(ticker, 'sell', 1, sellPrice, `LP-${ticker}`);
    price = y / x;
  } else {
    sellPrice = '-';
    price = 999999999.99;
  ...
  if (y > buyPrice) {
    await addOrder(ticker, 'buy', 1, buyPrice, `LP-${ticker}`);
  } else {
    buyPrice = '-';
  }
...
const updateUserInventory = (userId, ticker, quantity, action) => {
  ...
  if (action === 'buy') {
    ...
    console.log(`Removing ${quantity} of ${ticker} to user ${userId} inventory`);
    if (stock) {
      stock.quantity -= quantity;
      if (stock.quantity <= 0) {
...
router.post('/data/order', async (req, res) => {
  ...
  try {
    await addOrder(ticker, orderType, quantity, price, userId);
    res.status(200).send(`Order created successfully`);
    console.log(`User ${userId} created ${orderType} order for ${ticker} of ${quantity} at price of ${price}`);
...
const addOrder = async (ticker, action, quantity, price, userId) => {
  ...
  if (remainingQuantity > 0) {
    ...
    console.log(`Function: User ${userId} created ${action} order for ${ticker} of ${remainingQuantity} at price of ${price}, date: ${date}`);
...
const removeOrder = async (stock, action, quantity, price, userId) => {
  const orderDir = path.resolve(__dirname, `../orders/${stock}`);
  const orderFile = path.join(orderDir, `${action}.csv`);
  ...
  existingOrders = existingOrders.map(order => {
    if (!orderFound && 
      ...
      //console.log(`Previous q: ${order.q}, new quantity: ${newQuantity}`);
      orderFound = true;
      if (newQuantity > 0) {
